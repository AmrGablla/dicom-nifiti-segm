...
const segmentation = cornerstoneTools.segmentation;
...
function getLabelMaps(element) {
  ...
  const segmentationModule = cornerstoneTools.segmentation;
  ...
  for (let i = 0; i < labelmaps3D.length; i++) {
    ...
    const metadata =
      labelmap3D && labelmap3D.metadata && labelmap3D.metadata.data
        ...
        : null;
    const colorLutTable =
      segmentationModule.state.addColorLUT(labelmap3D.colorLUTIndex);
...
function createSegment(
  ...
  if (newLabelMap) {
    ...
    segmentation.activeLabelmapIndex(element, nextLabelmapIndex);
    segmentation.setActiveSegmentation(viewportId, nextLabelmapIndex);
  }
